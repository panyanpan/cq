...
(function () {
    ...
    function beginTimer() {
        ...
        para_intervalIdMain = setInterval(async () => {
            ...
            if (para_globalBool) {
                ...
                if (config != null && config.length > 0) {                    
                    eval(p_TimeGotoMap(config));
                    console.log(`testTime-config:${config}][${new Date().toLocaleString()}]`);
                }
        ...
        p_alert_success('开始辅助');
    }
    function stopTimer() {
        if (para_intervalIdMain) {
            clearInterval(para_intervalIdMain);
            para_intervalIdMain = null;
    ...
    function beginTimer_f_Yiji() {
        ...
        p_alert_success('开始遗迹，time-:' + new Date().toLocaleString());
    }
    function stopTimer_f_Yiji() {
        ...
        if (para_intervalIdYiji) {
            ...
            p_alert_success('定时器已关闭time-遗迹:' + new Date().toLocaleString());
        } else {
            console.log('暂无运行中的定时器time-Yiji:' + new Date().toLocaleString());
            p_alert_error('暂无运行中的遗迹任务，time:' + new Date().toLocaleString());
        }
    }
    ...
    function stopTimer_f_Common() {
        ...
        switch (parseInt(p_select)) {
            case 0: stopTimerID("para_intervalIdBlood"); break;
    ...
    //         const config = checkboxList.map(sel => ({
    //             value: sel.value
    //         }));
    //         GM_setValue("p_YijiSelectConfig", config);
    //     }

    //     function loadYijiConfig() {
    //         const config = GM_getValue("p_YijiSelectConfig");
    //         if (!config) return;

    //         config.forEach((item, index) => {
    //             if (checkboxList[index]) {
    //                 checkboxList[index].value = item.value;
    //             }
    //         });
    //     }
    ...
    //坐标转换
    function f_ConvertXY(srcW, srcH, x, y, tarW, tarH) {//2160*1350(1440*900--1.5)  1024*768
        const newX = Math.round(x * (tarW / srcW));
        const newY = Math.round(y * (tarH / srcH));
