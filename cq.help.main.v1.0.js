...
(function() {
    ...
    //--------//--------//--------//--------//--------//--------//--------//--------
    var obj_timer ={para_intervalIdMain: null, para_intervalIdBlood: null, para_IntervalId: null};//-------------------------------------------------------------testtesttesttest
    ...
    function beginTimer() {
        ...
        intervalIdPYMain = setInterval(async () => {
            ...
            if(para_globalBool){
                ...
                if(nowHourPY >= 1225 && nowHourPY < 1429 && gd.map.curMapId != 200082){
                    Logic.deliverToFindNpc(600184);      //xiangyao15  200082
                    console.log("gotoMapTime1200:"+new Date().toLocaleString());
                }
                if(nowHourPY >= 1429 && nowHourPY < 1500 && gd.map.curMapId != 600){
                    console.log("gotoMapTimeBaidu:"+new Date().toLocaleString());
                    Logic.deliverToFindNpc(600136);//baigu2  600
                }
                if(nowHourPY >= 1500 && nowHourPY < 1800 && gd.map.curMapId != 200082){
                    Logic.deliverToFindNpc(600184);      //xiangyao15  200082
                    console.log("gotoMapTime1200:"+new Date().toLocaleString());
                }
                ...
                if(nowHourPY >= 2000 && nowHourPY < 2159 && gd.map.curMapId != 7155){
                    //Logic.deliverToFindNpc(600276);  //shengdi5-el  7155
                    //console.log("gotoMapTimeBaidu:"+new Date().toLocaleString());
                }
                if(nowHourPY >= 1800 && nowHourPY < 2100 && gd.map.curMapId != 5614){
                    Logic.deliverToFindNpc(800211);  //wuxianshilian2  5614
                    console.log("gotoMapTimeBaidu:"+new Date().toLocaleString());
                }
                if(nowHourPY >= 2200 && nowHourPY < 2359 && gd.map.curMapId != 200090){
                    //Logic.deliverToFindNpc(200090);//wujin1
                    //console.log("gotoMapTime2000:"+new Date().toLocaleString());
                }
                if(nowHourPY >= 2200 && nowHourPY < 2359 && gd.map.curMapId != 200056){
                    Logic.deliverToFindNpc(600100);      //盘恒    200056
                    console.log("gotoMapTime2000:"+new Date().toLocaleString());
                }
                if(nowHourPY >= 0 && nowHourPY < 1100 && gd.map.curMapId != 200056){
                    //Logic.deliverToFindNpc(600100);      //盘恒    200056
                    console.log("gotoMapTime2000:"+new Date().toLocaleString());
                }
                if(nowHourPY >= 0 && nowHourPY < 1145 && gd.map.curMapId != 200090){
                    //Logic.deliverToFindNpc(200090);//wujin1
                    //console.log("gotoMapTime2000:"+new Date().toLocaleString());
                }
                if(nowHourPY >= 0 && nowHourPY < 1145 && gd.map.curMapId != 200072){
                    //Logic.deliverToFindNpc(600141);//fudi1  200072
    ...
    function saveMapConfig() {
        const config = p_selectElements.map(sel => ({
    ...
    var para_yiji = [670,470];//670--------------------
    var para_yiji_mapid = [8036,8024];//mapid------------------
    function beginTimer_f_Yiji(){
        ...
        para_intervalIdYiji = setInterval(async () => {
            ...
            //console.log(`Time-yiji-arry:[${para_yiji}][${para_yiji_mapid}]`);
    ...
    //坐标转换
    function f_ConvertXY(x, y, tarW, tarH) {//2160*1350(1440*900--1.5)  1024*768  800*600
        const newX = Math.round(x * (tarW / 1440));
        const newY = Math.round(y * (tarH / 900));
